...
def test_image_generation_max_5():
    """Test 3.2: Image generation limited to 5 images"""
    response = requests.post(
        f"{API_URL}/chat",
    ...
    assert image_count <= 5, f"Expected max 5 images, got {image_count}"
    return f"Image count: {image_count} (max 5)"
